...
SELECT 
  ...
  COUNT(*) OVER (PARTITION BY sub.ir4_Tier_Level,sub.Student_Grade) AS ir4_Tier_Level_Count, 
  CASE -- Creating index for ordering
    ...
    END AS iReady_Index,
  sub.Student_Grade,
  ROUND(COUNT(*) OVER (PARTITION BY  sub.ir4_Tier_Level,sub.Student_Grade) / COUNT(*) OVER (PARTITION BY sub.Student_Grade),2) AS ir4_Percentage,
  COUNT(*) OVER (PARTITION BY sub.Student_Grade) AS Total
FROM
(SELECT
  CAST(REPLACE(Student_Grade,'K','0')AS numeric) AS Student_Grade, 
...
UNION ALL
SELECT
  ir4.iReady_Index, ir4.ir4_Tier_Level AS Tier_Level,ir4.ir4_Tier_Level_Count,ir4.Total,ir4_Percentage AS Percentage, ir4.student_grade
