...
SELECT 
  ...
  sub.Campus,
  COUNT(*) OVER (PARTITION BY sub.ir1_Tier_Level,sub.Campus) AS Tier_Level_Count, 
  ...
  WHEN sub.ir1_Tier_Level = 'Tier 3' THEN 1.08
    END AS iReady_Index,  
  ROUND(COUNT(*) OVER (PARTITION BY sub.ir1_Tier_Level,sub.Campus) / COUNT(*) OVER (PARTITION BY sub.Campus),2) AS ir1_Percentage,
...
(SELECT
  ...
  CASE -- Creating proficiency CASE
    ...
    END AS proficiency,
FROM `my-data-project-36654.iReady_SY_2223.iReady_ELA_1`) AS sub -- subquery
...
SELECT 
  ...
  COUNT(*) OVER (PARTITION BY sub.ir2_Tier_Level, sub.Campus) AS ir2_Tier_Level_Count, 
   CASE -- Creating index for ordering
  ...
  END AS iReady_Index,
  sub.Campus,
  ROUND(COUNT(*) OVER (PARTITION BY  sub.ir2_Tier_Level,sub.Campus) / COUNT(*) OVER (PARTITION BY sub.Campus),2) AS ir2_Percentage,
  COUNT(*) OVER (PARTITION BY sub.Campus) AS Total,
FROM
(SELECT
  CASE 
    WHEN Student_Grade IN ('K','1','2','3','4','5') THEN 'ES' -- used this method due to Grade Level being string data type
    WHEN Student_Grade IN ('6','7','8') THEN 'MS'
    ELSE 'HS'
    END AS Campus,
  CONCAT(First_Name,' ',Last_Name) AS Name,
  ...
  CASE -- Creating Proficiency CASE
    ...
    ELSE 'Not On Grade Level' 
    END AS proficiency
FROM `my-data-project-36654.iReady_SY_2223.iReady_2_ELA`) AS sub -- subquery
...
SELECT 
  ...
  COUNT(*) OVER (PARTITION BY sub.ir3_Tier_Level,sub.Campus) AS ir3_Tier_Level_Count, 
  CASE -- Creating index for ordering
  ...
  WHEN sub.ir3_Tier_Level = 'Tier 3' THEN 1.10
    END AS iReady_Index,
  sub.Campus,
  ROUND(COUNT(*) OVER (PARTITION BY  sub.ir3_Tier_Level,sub.Campus) / COUNT(*) OVER (PARTITION BY sub.Campus),2) AS ir3_Percentage,
  COUNT(*) OVER (PARTITION BY sub.Campus) AS Total
FROM
(SELECT
  CASE 
  ...
  CASE -- Creating Profiency CASE
    ...
    ELSE 'Not On Grade Level' 
    END AS proficiency
FROM `my-data-project-36654.iReady_SY_2223.iReady_3_ELA`) AS sub --subquery
...
SELECT 
  ...
  COUNT(*) OVER (PARTITION BY sub.ir4_Tier_Level,sub.Campus) AS ir4_Tier_Level_Count, 
  CASE -- Creating index for ordering
  ...
  WHEN sub.ir4_Tier_Level = 'Tier 3' THEN 1.11
    END AS iReady_Index,
  sub.Campus,
  ROUND(COUNT(*) OVER (PARTITION BY  sub.ir4_Tier_Level,sub.Campus) / COUNT(*) OVER (PARTITION BY sub.Campus),2) AS ir4_Percentage,
  COUNT(*) OVER (PARTITION BY sub.Campus) AS Total
FROM
(SELECT
  CASE 
...
/*Combining all CTEs into single dataset*/
  SELECT
  ir1.iReady_Index, ir1.ir1_Tier_Level AS Tier_Level,ir1.Tier_Level_Count,ir1.Total,ir1_Percentage AS Percentage, ir1.Campus
FROM ir1 --iReady 1 Data
UNION ALL
SELECT
  ir2.iReady_Index, ir2.ir2_Tier_Level AS Tier_Level,ir2.ir2_Tier_Level_Count,ir2.Total,ir2_Percentage AS Percentage, ir2.Campus
FROM ir2 -- iReasdy 2 Data
UNION ALL
SELECT
  ir3.iReady_Index, ir3.ir3_Tier_Level AS Tier_Level,ir3.ir3_Tier_Level_Count,ir3.Total,ir3_Percentage AS Percentage, ir3.Campus
FROM ir3 -- iReady 3 Data
UNION ALL
SELECT
  ir4.iReady_Index, ir4.ir4_Tier_Level AS Tier_Level,ir4.ir4_Tier_Level_Count,ir4.Total,ir4_Percentage AS Percentage, ir4.Campus
FROM ir4 -- iReady 4 Data
ORDER BY iReady_Index) AS sub -- 1st subquery
ORDER BY sub.iReady_Index) AS sub1 -- 2nd subquery
ORDER BY sub1.Campus, sub1.iReady_Index; 
~~~
